document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        const formatWeight = () => {
            if (!car.weight) return 'Nenurodyta';
            return String(car.weight).includes('kg') ? car.weight : `${car.weight} kg`;
        };

        const specItems = [
            { label: 'Pirma registracija', value: car.firstRegistration || 'Nenurodyta' },
            { label: 'Rida', value: `${car.mileage.toLocaleString('lt-LT')} km` },
            { label: 'Variklis', value: car.engine || 'Nenurodyta' },
            { label: 'Kuro tipas', value: car.fuel || 'Nenurodyta' },
            { label: 'Kėbulo tipas', value: car.body || 'Nenurodyta' },
            { label: 'Durų skaičius', value: car.doors || 'Nenurodyta' },
            { label: 'Varantieji ratai', value: car.drivetrain || 'Nenurodyta' },
            { label: 'Pavarų dėžė', value: car.transmission || 'Nenurodyta' },
            { label: 'Klimato valdymas', value: car.climateControl || 'Nenurodyta' },
            { label: 'Spalva', value: car.color || 'Nenurodyta' },
            { label: 'Vairo padėtis', value: car.steering || 'Kairėje' },
            { label: 'Tech. apžiūra iki', value: car.inspectionUntil || 'Nenurodyta' },
            { label: 'Nuosava masė, kg', value: formatWeight() },
            { label: 'Sėdimų vietų skaičius', value: car.seats || 'Nenurodyta' },
            { label: 'Pirmosios registracijos šalis', value: car.registrationCountry || 'Nenurodyta' },
            { label: 'VIN', value: car.vin || 'Pateikiama apžiūros metu' },
            { label: 'SDK', value: car.sdk || 'Pateikiama apžiūros metu' },
            { label: 'CO₂ emisija, g/km', value: car.co2 || 'Nenurodyta' },
        ];
